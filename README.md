...
> Note: Some users are served the chart on `chart/chart_tradingview.php` and this script will not work if your charts are loaded from there. It will only work on the `chart/chart.php` url. *To switch the chart souce, edit the option under [Profile](https://www.cryptohopper.com/edit-profile) > Settings > Chart source.*
